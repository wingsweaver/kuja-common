...
/**
 * 用于在 {@link AbstractEnvironmentResolver} 中做控制设置的 {@link EnvironmentPostProcessor} 实现类。<br>
 * 主要是设置一些参数，以抑制不必要的 Web 初始化和 Cloud Config 设置。也可以按需定义其他需要控制的参数。<br>
 * 不过注意需要在 {@link AbstractEnvironmentResolver} 中配合 {@link PostEnvironmentResolverProcessor} 来使用，
